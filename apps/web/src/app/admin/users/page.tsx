    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setInviteStatus({ type: 'error', message: data.error || 'Error al enviar invitación' });
            }
        } catch (error) {
        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p className="text-gray-600">Gestiona aprobaciones e invita a nuevos colaboradores.</p>
                </div>
            </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Aprobado</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {users.length === 0 ? (
                                        <tr>
                                            <td colSpan={4} className="px-6 py-10 text-center text-gray-500">No hay usuarios registrados.</td>
                                        </tr>
                                    ) : (
                                        users.map((user) => (
                                            <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    {user.role !== 'ADMIN' && (
                                                        ...
                                                        >
                                                            <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${user.approved ? 'translate-x-5' : 'translate-x-0'}`} />
                                                        </button>
                                                    )}
                                                </td>
                                            </tr>
                ...
                <div className="lg:col-span-1">
                    <Card>
                        <div className="p-2">
                            ...
                            <p className="text-sm text-gray-600 mb-6">
                                Envía una invitación por correo electrónico para que un nuevo usuario se registre.
                            </p>

                            <form onSubmit={handleInvite} className="space-y-4">
                                {inviteStatus && (
                                    <div className={`p-3 rounded-lg text-sm ${inviteStatus.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    ...
                    <div className="mt-6 bg-blue-50 p-4 rounded-xl border border-blue-100">
                        ...
                        <ul className="text-xs text-blue-800 space-y-2">
                            <li>• Los usuarios invitados son aprobados automáticamente al registrarse.</li>
                            <li>• En modo <strong>Whitelist</strong>, los registros libres requieren aprobación manual aquí.</li>
                            <li>• En modo <strong>Invite-only</strong>, el botón de registro público está deshabilitado.</li>
                        </ul>
                    </div>
                </div>
            </div>
